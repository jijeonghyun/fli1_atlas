#Bulk ATACseq + CUT&Tag Essentials
...
#data <- read.csv("raw_counts.csv", row.names = "Geneid") #For .csv files
data <- read.delim(file="featureCounts.txt", sep="\t", stringsAsFactors=FALSE, header=TRUE, row.names = "Peakid") #For tab delimited txt files
#Use this to select the columns you want
colnames(data)
data <- data[,c(6,7,8,9,10,11)] #this is to ID the columns you want to use for analysis#
colnames(data)
...
condition <- c(rep("Fli1KO", 3), rep("NTC", 3))
...
annotation <- read.delim(file="annoPeaks.txt", sep="\t", stringsAsFactors=FALSE, header=TRUE, row.names = "Peakid") #For tab delimited txt files
annotation <- annotation[,c(1,2,3,7,9,15,17,18)]
normalized_counts <- as.data.frame(normalized_counts)
#normalized_counts <- rownames_to_column(as.data.frame(normalized_counts), var = "Row.names")
#annotated_data <- right_join(annotation, normalized_counts, by = c("Peak.Score" = "Row.names"))
annotated_data <- merge(annotation, normalized_counts, by = 'row.names', all = TRUE)

#Ues below code if you want to change the column "Row.names" to be the actual row names
#annotated_data <- annotated_data %>% remove_rownames %>% column_to_rownames(var="Row.names")
write.csv(annotated_data, file = "Mouse_Fli1vRosa_ATAC_counts.csv")
#Make a pie chart
pie1(table(annotated_data$Annotation))
...
generate_DE_results <- function (dds, comparisons, padjcutoff = 0.05, log2cutoff = 0.5, cpmcutoff = 2) {
  ...
  colnames(cpms) <- c("Peakid", "avg_cpm")
  comparisons <- c("NTC", "Fli1KO")
  ...
  res <- as_tibble(res, rownames = "Peakid")
  # read in the annotation and append it to the data
  annotation <- read.delim(file="annoPeaks.txt", sep="\t", stringsAsFactors=TRUE, header=TRUE) #For tab delimited txt files
  annotation <- annotation[,c(1,2,3,4,8,10,16,18,19)]
  #Merge annotation and cpms together
  annotation <- merge(annotation, cpms, by = c("Peakid" = "Peakid"))
  #Merge res to the annotation file
  res <- merge(annotation, res, by = c("Peakid" = "Peakid"))
  ...
  combined_data <- merge(res, normalized_counts, by = c("row.names"))
  #combined_data <- as_tibble(cbind(annotation, normalized_counts[,grep(pattern, colnames(normalized_counts))] ))
  #combined_data <- as_tibble(cbind(combined_data, res))
  ...
  res_prot <- res[which(res$Gene.Type=="protein-coding"),]
...
DE_output <- generate_DE_results (dds, c("Fli1KO", "NTC"), padjcutoff = 0.9, log2cutoff = 0.1, cpmcutoff = 0.1)
...
res <- read.csv("NTC_vs_Fli1KO_allgenes.csv", header = T)

DE_gene_heatmap <- function(res, padj_cutoff = 0.1, ngenes = 40) {
  ...
  heatmap_values <- as.matrix(significant_genes[,c(16,17,18,19,20,21)])
...
plot_counts <- function (dds, gene, normalization = "DESeq2"){
  ...
  annotation <- read.delim(file="annoPeaks.txt", sep="\t", stringsAsFactors=TRUE, header=TRUE, row.names = "Peakid") #For tab delimited txt files
  ...
  if (is.numeric(gene)) { # check if an index is supplied or if ensembl_id is supplied
    ...
    ensembl_id <- annotation$Gene.name[which(annotation$Gene.name == gene)]
